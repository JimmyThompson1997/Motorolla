...
def test_validate_browser_evidence_accepts_live_user_session_schema(tmp_path: Path) -> None:
    local = {"head": "abcdef", "head_short": "abcdef0"}
    remote = {
        "schema": "pucky.ui_bundle.v1",
        "ui_version": "git-abcdef0",
    ...
    evidence = {
        "schema": "pucky.live_user_session_browser_proof.v1",
        "ok": True,
        "source_commit_full": "abcdef",
        "source_commit_short": "abcdef0",
        "ui_version": "git-abcdef0",
    ...
    path = tmp_path / "browser-live-user-session.json"
    path.write_text(json.dumps(evidence), encoding="utf-8")

    validated = official.validate_browser_evidence(official.load_browser_evidence(path), remote, local)

    assert validated["schema"] == "pucky.live_user_session_browser_proof.v1"
...
def test_task_live_browser_proof_source_records_manifest_identity_and_refresh_key() -> None:
    source = (
        Path(official.__file__).resolve().parent
        / "proofs"
        / "workspace"
