...
function linkedTargetLocator(page, kind) {
  if (kind === "note") {
    return page
      .locator(".light-info-section")
      .filter({ has: page.locator(".light-section-title", { hasText: "NOTES" }) })
      .locator('.light-info-row[data-workspace-target-kind="note"]')
      .first();
  }
  return page.locator(`.light-info-row[data-task-attachment-kind="${kind}"]`).first();
}

async function clickChipForKind(page, kind, timeoutMs) {
  const locator = linkedTargetLocator(page, kind);
