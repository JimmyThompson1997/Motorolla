...
function parseArgs(argv) {
  const config = {
    ...
    requireDistinctWorkspaceHost: truthy(process.env.PUCKY_AUTH_REQUIRE_DISTINCT_WORKSPACE_HOST, true),
...
async function logout(page, loginUrl, logoutLabels, timeoutMs) {
  const label = await maybeClickOneOf(page, logoutLabels, 4000);
...
async function runIsolationChecks(browser, matrixEntry, config, laneDir, ownerWorkspaceUrl, seed, artifactId, ownerPreLogoutState) {
  ...
  const logoutResult = await logout(ownerPage, config.loginUrl, config.logoutLabels, config.timeoutMs);
  ...
  try {
    for (const matrixEntry of config.matrix) {
      ...
      try {
        ...
        const logoutResult = await logout(page, config.loginUrl, config.logoutLabels, config.timeoutMs);
        lane.user_a.logout = {
          ...logoutResult,
