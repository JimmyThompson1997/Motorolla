...
def composio_app_env_overrides(app_slug: str) -> dict[str, str]:
    ...
    if app_slug == "googlecalendar" and "PUCKY_COMPOSIO_REQUIRE_VERIFICATION_COMMAND" not in env:
...
def ensure_composio_app_inputs(app_slug: str) -> None:
    ...
    if app_slug == "googlecalendar" and not composio_app_setting(app_slug, "USER_A_VERIFY_COMMAND"):
        raise ReleaseGauntletError(
