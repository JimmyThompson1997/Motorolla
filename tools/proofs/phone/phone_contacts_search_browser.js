...
async function ensureContactsRoute(client, request) {
  ...
  return readContactsState(client);
}

async function installContactsTrace(client) {
...
async function readContactsState(client) {
  return client.evaluate(`(() => {
    const shell = document.querySelector(".light-shell");
    const search = document.querySelector(".light-contacts-search");
...
async function searchInputCenter(client) {
  return client.evaluate(`(() => {
...
async function readContactsTrace(client) {
  return client.evaluate(`(() => {
    const store = window.__contactsSearchTraceStore || {};
    const current = document.getElementById("contactsSearch");
...
async function runOperation(client, request, operation) {
  ...
  if (kind === "install_contacts_trace") {
    return installContactsTrace(client);
  }
  if (kind === "read_contacts_state") {
    return readContactsState(client);
  }
  if (kind === "search_input_center") {
    return searchInputCenter(client);
  }
  if (kind === "read_contacts_trace") {
    return readContactsTrace(client);
  }
  if (kind === "screenshot") {
    return { path: await client.screenshot(String(operation.path || "")) };
  }
  throw new Error(`Unsupported contacts-search browser operation: ${kind || "<missing>"}`);
}
...
async function main() {
  ...
  if (!payload.ok) {
    throw new Error(String(payload.error || "contacts-search browser helper failed"));
  }
}
