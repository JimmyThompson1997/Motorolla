...
DEFAULT_HELPER = ROOT / "tools" / "proofs" / "phone" / "phone_contact_detail_classic_edit_browser.js"
DEFAULT_REPORT_DIR = ROOT / ".tmp" / "proof-live-contact-detail-classic-edit-emulator"
CHROME_PACKAGE = "com.android.chrome"


class ContactDetailClassicEditEmulatorProofError(RuntimeError):
    pass


def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Prove classic contact-detail edit typing stability on the Android emulator."
    )
    ...
    parser.add_argument("--base-url", default=os.environ.get("PUCKY_CONTACT_DETAIL_EMULATOR_BASE_URL", DEFAULT_BASE_URL))
    parser.add_argument("--api-token", default=os.environ.get("PUCKY_API_TOKEN", ""))
    parser.add_argument("--refresh-key", default=os.environ.get("PUCKY_CONTACT_DETAIL_EMULATOR_REFRESH", ""))
...
def resolve_emulator_serial(args: argparse.Namespace) -> str:
    ...
    if preferred:
        if preferred not in serials:
            raise ContactDetailClassicEditEmulatorProofError(
                f"Expected emulator {preferred}, saw {serials or ['<none>']}"
            )
        return preferred
    if len(serials) == 1:
        return serials[0]
    if not serials:
        raise ContactDetailClassicEditEmulatorProofError(
            "No running Android emulator was available for the contact-detail classic-edit proof."
        )
    raise ContactDetailClassicEditEmulatorProofError(
        f"Multiple emulators are running; pass --serial explicitly ({serials})"
    )
...
def ensure_runtime(args: argparse.Namespace) -> None:
    if not args.api_token:
        raise ContactDetailClassicEditEmulatorProofError(
            "Classic contact-detail emulator proof requires --api-token or PUCKY_API_TOKEN."
        )
    if not args.browser_helper.exists():
        raise ContactDetailClassicEditEmulatorProofError(f"Browser helper not found: {args.browser_helper}")
    if not Path(args.node).exists():
        raise ContactDetailClassicEditEmulatorProofError(f"Node executable not found: {args.node}")
...
def fetch_manifest(args: argparse.Namespace) -> dict[str, Any]:
    ...
    return {"url": manifest_url, "payload": payload}
...
def api_request(args: argparse.Namespace, method: str, path: str, body: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    try:
        ...
        raise ContactDetailClassicEditEmulatorProofError(f"{method.upper()} {path} failed ({exc.code}): {detail}") from exc


def create_proof_contact(args: argparse.Namespace, run_id: str) -> str:
    contact_id = f"{run_id}-android-edit-contact"
    api_request(
        args,
        "POST",
        ...
        {
            "id": contact_id,
            "title": "Proof Android Contact",
            "summary": "Keyboard stability proof contact",
            "metadata": {
                "first_name": "",
                "last_name": "Proof Android Contact",
                "email": "android.proof.contact@example.com",
                "phone": "+1 (415) 555-0144",
                "activity": ["Linked to emulator alpha"],
            },
        },
    )
    return contact_id
...
def delete_proof_contact(args: argparse.Namespace, contact_id: str) -> bool:
    if not str(contact_id or "").strip():
        return True
    try:
        api_request(args, "DELETE", f"/api/workspace/contacts/{contact_id}")
        return True
    except Exception:
        return False
    ...
    while time.monotonic() < deadline:
        ...
        for socket_name in sockets:
            ...
            try:
                ...
                matches = [page for page in (pages or []) if "/ui/pucky/latest" in str(page.get("url") or "")]
                if matches:
                    keep_forward = True
                    return {
                        "socket": socket_name,
...
def ime_dumpsys(args: argparse.Namespace) -> str:
    outputs = []
    for adb_args in (["shell", "dumpsys", "input_method"], ["shell", "dumpsys", "window"]):
        completed = run(args, adb_args, timeout=20, check=False)
...
def wait_for_ime_visible(args: argparse.Namespace, timeout_seconds: float) -> str:
    ...
    while time.monotonic() < deadline:
        ...
        if ime_visible_from_dumpsys(last_dump):
            return last_dump
        time.sleep(0.35)
    raise ContactDetailClassicEditEmulatorProofError("Timed out waiting for the Android IME to stay visible.")
...
def dump_uiautomator_xml(args: argparse.Namespace) -> str:
    ...
    while time.monotonic() < deadline:
        ...
        time.sleep(0.25)
    raise ContactDetailClassicEditEmulatorProofError(last_error)


def parse_bounds(raw: str) -> tuple[int, int, int, int]:
    match = re.match(r"\[(\d+),(\d+)\]\[(\d+),(\d+)\]", str(raw or "").strip())
    if not match:
        raise ContactDetailClassicEditEmulatorProofError(f"Invalid node bounds: {raw!r}")
...
def translate_webview_point(point: dict[str, Any], webview_bounds: tuple[int, int, int, int]) -> tuple[int, int]:
...
def helper_detail_ready(args: argparse.Namespace, chrome_cdp_url: str, contact_id: str) -> dict[str, Any]:
    return run_browser_helper(
        args,
        cdp_url=chrome_cdp_url,
        surface="contact_detail_classic_edit",
        page_url_contains="/ui/pucky/latest",
        operations=[
            {"kind": "ensure_contacts_route"},
            {"kind": "set_contacts_search_query", "query": "Proof Android Contact"},
            {"kind": "open_contact_detail", "contact_id": contact_id},
            {"kind": "enter_contact_edit_mode"},
            {"kind": "install_contact_edit_trace", "field": "first_name"},
            {"kind": "contact_edit_field_center", "field": "first_name"},
            {"kind": "read_contact_detail_state"},
            {"kind": "screenshot", "path": str(args.report_dir / "contact-detail-cdp.png")},
        ],
    )
...
def helper_read_state_and_trace(args: argparse.Namespace, chrome_cdp_url: str) -> dict[str, Any]:
    return run_browser_helper(
        ...
        operations=[
            {"kind": "read_contact_detail_state"},
            {"kind": "read_contact_edit_trace", "field": "first_name"},
        ],
    )
...
def wait_for_first_name(args: argparse.Namespace, chrome_cdp_url: str, expected_value: str, timeout_seconds: float) -> dict[str, Any]:
    ...
    while time.monotonic() < deadline:
        ...
        time.sleep(0.25)
    raise ContactDetailClassicEditEmulatorProofError(
        f"Timed out waiting for first name {expected_value!r}; last state was {json.dumps(last or {}, indent=2)}"
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    summary: dict[str, Any] = {
        ...
        "trace_json": str(args.report_dir / "contact-detail-edit-trace.json"),
        "screenshots": {},
        "first_name_progression": [],
        "proof_contact_id": "",
        "cleanup_ok": False,
    }
    try:
        summary["manifest"] = verify_manifest(args)
        proof_contact_id = create_proof_contact(args, run_id)
        summary["proof_contact_id"] = proof_contact_id
        ensure_device(args)
        ...
        ready = helper_detail_ready(args, str(chrome["cdp_url"]), proof_contact_id)
        summary["detail_ready"] = ready
        field_point = ready.get("results", [{}, {}, {}, {}, {}, {"result": None}])[5].get("result")
        detail_state = ready.get("results", [{}, {}, {}, {}, {}, {}, {"result": {}}])[6].get("result") or {}
        if not field_point:
            raise ContactDetailClassicEditEmulatorProofError("Could not read first-name field geometry from Chrome DevTools.")
        if str(detail_state.get("route") or "") != "contact-detail":
            raise ContactDetailClassicEditEmulatorProofError(
                f"Expected hosted Chrome route contact-detail, got {detail_state.get('route')!r}"
            )
        if str(detail_state.get("mode") or "") != "edit":
            raise ContactDetailClassicEditEmulatorProofError(
        ...
        tap(args, *tap_point)
        ime_before = wait_for_ime_visible(args, 10)
        write_text(args.report_dir / "ime-before.txt", ime_before)
        capture_screenshot(args, args.report_dir / "contact-detail-keyboard-before.png", "contact-detail-keyboard-before.png")
        summary["screenshots"]["before"] = str(args.report_dir / "contact-detail-keyboard-before.png")
        summary["field_center"] = {
        ...
        prefixes = [
            ("d", "contact-detail-keyboard-after-d.png"),
            ("da", "contact-detail-keyboard-after-da.png"),
            ("dav", "contact-detail-keyboard-after-dav.png"),
        ]
        for expected_value, screenshot_name in prefixes:
            ...
            tap(args, *key_center)
            state_trace = wait_for_first_name(args, str(chrome["cdp_url"]), expected_value, 12)
            ...
            ime_path = args.report_dir / f"ime-after-{expected_value}.txt"
            write_text(ime_path, ime_dump)
            capture_screenshot(args, args.report_dir / screenshot_name, screenshot_name)
            summary["screenshots"][expected_value] = str(args.report_dir / screenshot_name)
            summary["first_name_progression"].append(
                {
                    "expected_value": expected_value,
                    "first_name": current_state.get("firstName"),
                    "focus": focus,
            ...
            if not focus.startswith(f"{CHROME_PACKAGE}/"):
                raise ContactDetailClassicEditEmulatorProofError(
                    f"Chrome lost focus while typing {expected_value!r}: {focus or '<none>'}"
                )
        ...
        write_json(args.report_dir / "contact-detail-edit-trace.json", final_trace)
        ...
        if (event_counts.get("contact-edit-node-changed") or 0) != 0:
            raise ContactDetailClassicEditEmulatorProofError(
                f"Expected no contact-detail input replacement while typing dav; saw {json.dumps(event_counts, indent=2)}"
            )
        ...
        summary["cleanup_ok"] = delete_proof_contact(args, proof_contact_id)
        write_json(args.report_dir / "summary.json", summary)
        if forward_port:
