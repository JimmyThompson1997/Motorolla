...
def run(args: argparse.Namespace, adb_args: list[str], *, timeout: int = 30, check: bool = True) -> subprocess.CompletedProcess[str]:
    command = [str(args.adb), "-s", args.serial, *adb_args]
...
def dismiss_permission_dialogs(args: argparse.Namespace, *, timeout_seconds: int = 20) -> None:
    ...
    while time.time() < deadline:
        ...
        for label in dismiss_labels:
            matched = find_node_by_text(root, label)
...
def capture_screenshot(args: argparse.Namespace, target: Path, remote_name: str) -> None:
    ...
    completed = run(args, ["pull", f"/sdcard/{remote_name}", str(target)], timeout=30)
    if completed.returncode != 0:
        raise EmulatorLinksProofError(f"Could not pull screenshot to {target}")
...
def is_cover_page(page: dict[str, Any]) -> bool:
    ...
    return "Pucky Cover" in title or "index.html" in url
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        connect_screen = args.report_dir / "01-connect-device.png"
        capture_screenshot(args, connect_screen, "links-proof-connect-device.png")
        summary["screenshots"]["connect_device"] = str(connect_screen)
        ...
        browser_screen = args.report_dir / "04-auth-device.png"
        capture_screenshot(args, browser_screen, "links-proof-auth-device.png")
        summary["screenshots"]["auth_device"] = str(browser_screen)
