...
const scenarios = [
  ...
  { key: "new_chat_smoke", token: `${RUN_PREFIX}-SMOKE-1`, expected: `ACK ${RUN_PREFIX}-SMOKE-1` },
  { key: "continuation_smoke", token: `${RUN_PREFIX}-CONT-1`, expected: `ACK ${RUN_PREFIX}-CONT-1` },
...
async function clickHeaderCompose(page, timeoutMs) {
  await page.locator('[aria-label="Compose new chat"]').first().click();
  await page.waitForFunction(
    () => {
      const detail = document.getElementById("detail");
      return Boolean(detail && detail.getAttribute("aria-hidden") === "false" && detail.querySelector(".thread-composer-input"));
    },
    undefined,
    { timeout: timeoutMs },
  );
...
async function findThreadTarget(page, options = {}) {
  const target = await page.evaluate((input) => {
    const preferredThreadId = String(input?.threadId || "").trim();
    ...
    return mapped.find(card => preferredThreadId && card.threadId === preferredThreadId)
      || mapped.find(card => card.title === "Thread Compose Seed")
...
async function openPendingTranscriptForToken(page, token, timeoutMs) {
  await page.evaluate((expectedToken) => {
    const cards = Array.from(document.querySelectorAll("article.card-outbound"));
    const target = cards.find(card => String(card.querySelector(".card-outbound-preview")?.textContent || "").includes(expectedToken));
    if (!target) {
      throw new Error(`Pending outbound card not found for token ${expectedToken}`);
    }
    target.click();
  }, token);
  await page.waitForFunction(
...
async function waitForThreadDetailThreadId(page, timeoutMs) {
  await page.waitForFunction(
    () => {
      const detail = document.getElementById("detail");
      return Boolean(detail && detail.getAttribute("aria-hidden") === "false" && String(detail.getAttribute("data-detail-thread-id") || "").trim());
    },
    undefined,
    { timeout: timeoutMs },
  );
  const snapshot = await composerSnapshot(page);
  return String(snapshot.thread_id || "").trim();
}
...
async function runClosedLaneScenario(page, files, scenario, config, summary) {
  await clickHeaderCompose(page, config.timeoutMs);
  ...
  const snapshot = await composerSnapshot(page);
  assert(snapshot.type === "transcript_new", `Closed-lane compose should open the blank new-thread detail. Saw: ${snapshot.type}`);
...
async function runOpenLaneScenario(page, files, config, summary, networkEvents) {
  summary.header_compose = true;
  await clickHeaderCompose(page, config.timeoutMs);
  await saveScreenshot(page, config.reportDir, "03-open-lane-compose-idle");

  const smoke = scenarios.find(item => item.key === "new_chat_smoke");
  assert(smoke, "New-chat smoke scenario is missing.");
  await page.evaluate(() => {
  ...
  await openPendingTranscriptForToken(page, smoke.token, config.timeoutMs);
  await waitForBubbleText(page, "Thinking...", config.timeoutMs);
  await saveScreenshot(page, config.reportDir, "06-thinking-detail");
  const createdThreadIdPromise = waitForThreadDetailThreadId(page, config.timeoutMs);
  ...
  assert(summary.new_chat_smoke.final.thread_id === createdThreadId, "Created thread detail should resolve to the same thread id returned after the first new-chat send.");
  await saveScreenshot(page, config.reportDir, "07-new-thread-final");
  await page.evaluate(() => {
  ...
  const continuationReplyPromise = page.waitForResponse(
    response => response.request().method() === "POST"
      && /\/api\/turn\/text(?:$|[?#])/.test(response.url())
      && String(response.request().postData() || "").includes(continuation.token),
    { timeout: config.timeoutMs * 2 },
  );
  ...
  summary.continuation_smoke = {
    final: await composerSnapshot(page),
    turn_response: {
  ...
  assert(summary.continuation_smoke.turn_response.requested_thread_id === createdThreadId, "Continuation send should reuse the thread created by the new-chat first turn.");
  await saveScreenshot(page, config.reportDir, "08-continuation-final");
  ...
  const blockedReplyPromise = page.waitForResponse(
    response => response.request().method() === "POST"
      && /\/api\/turn\/text(?:$|[?#])/.test(response.url())
      && String(response.request().postData() || "").includes(blocked.token),
  ...
  await saveScreenshot(page, config.reportDir, "09-blocked-second-send");
  ...
  await page.waitForFunction(
    (draftToken) => {
      ...
      return Boolean(textarea && send && String(textarea.value || "").includes(draftToken) && !send.disabled);
    },
    `${RUN_PREFIX}-BLOCK-2`,
  ...
  const secondReplyPromise = page.waitForResponse(
    response => response.request().method() === "POST"
      && /\/api\/turn\/text(?:$|[?#])/.test(response.url())
      && String(response.request().postData() || "").includes(`${RUN_PREFIX}-BLOCK-2`),
    { timeout: config.timeoutMs * 2 },
  );
  await clickSend(page);
  ...
  await queueComposerFiles(page, [files.notePath]);
  await saveScreenshot(page, config.reportDir, "10-text-attachment-queued");
  await clickSend(page);
  ...
  await saveScreenshot(page, config.reportDir, "11-text-attachment-open");
  ...
  await saveScreenshot(page, config.reportDir, "12-image-attachment-queued");
  await clickSend(page);
  ...
  await saveScreenshot(page, config.reportDir, "13-image-attachment-open");
  await page.locator("#detail .light-back-button, #detail .detail-back").first().click();
  await page.waitForSelector("#detail .thread-composer");
  ...
  await saveScreenshot(page, config.reportDir, "14-existing-thread-draft-before-back");
  await closeDetail(page);
  const createdThreadTarget = await findThreadTarget(page, { threadId: createdThreadId });
  await clickCardTranscript(page, createdThreadTarget);
  const postBackSnapshot = await composerSnapshot(page);
  assert(postBackSnapshot.composer_text.includes(draftReturn.token), "Unsent existing-thread draft text did not survive leaving and returning to the thread.");
  assert(postBackSnapshot.chips.includes(THREAD_COMPOSE_NOTE), "Queued file chip did not survive leaving and returning to the existing thread.");
  summary.back_return = {
    before: preBackSnapshot,
    after: postBackSnapshot,
  };
  await saveScreenshot(page, config.reportDir, "15-existing-thread-back-return");
}
...
async function main() {
  ...
  try {
    ...
    const closedLane = await runContext(browser, config, closedLaneDir, async ({ page }) => {
      ...
      const closedSummary = {};
      await runClosedLaneScenario(page, fixtureFiles, scenarios[0], config, closedSummary);
    ...
    const createdThreadId = String(summary?.new_chat_smoke?.final?.thread_id || "").trim();
    summary.new_thread_created = Boolean(createdThreadId);
    summary.existing_thread_reused = Boolean(
      createdThreadId
      && summary?.continuation_smoke?.turn_response?.requested_thread_id === createdThreadId
    );
    summary.blocked_second_send_ok = Boolean(
      summary?.blocked_second_send?.final?.thread_id
      && summary.blocked_second_send.final.thread_id === createdThreadId
    );
