...
async function safeTextSnippet(page) {
  try {
...
async function safePageSnapshot(page) {
  return {
    ...
    text: await safeTextSnippet(page),
  };
}
  ...
  try {
    ...
    while (Date.now() < deadline) {
      if (popupPage) {
        ...
        if (!isIgnorableAuthUrl(popupUrl, connectUrl)) {
          return {
            kind: "popup",
            page: popupPage,
            snapshot: await safePageSnapshot(popupPage),
          };
        }
      }
      const pageUrl = page.url();
      if (!isIgnorableAuthUrl(pageUrl, connectUrl)) {
        return {
          kind: "same_tab",
          page,
...
async function main() {
  ...
  try {
    ...
    if (hasDebugContract && (!Array.isArray(metrics.filtered_slugs) || metrics.filtered_slugs.length === 0)) {
      throw new Error("Connect did not expose filtered slug debug state before search.");
    }
    ...
    if (hasDebugContract) {
      await page.waitForFunction(
        slug => {
          const metrics = window.PuckyUiDebug?.linksMetrics?.();
          return Boolean(Array.isArray(metrics?.filtered_slugs) && metrics.filtered_slugs.includes(slug));
        },
        config.appSlug,
        { timeout: config.timeoutMs }
      );
    } else {
      await page.locator(`.links-app-row[data-links-slug="${config.appSlug}"]`).first().waitFor({ state: "visible", timeout: config.timeoutMs });
    }
