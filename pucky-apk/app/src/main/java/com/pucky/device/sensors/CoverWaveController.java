...
public final class CoverWaveController {
    public interface Callbacks {
        void onCoverWaveWake(String reason);
    }

    private static final String TAG = "PuckyCoverWave";
    private static final String PREFS = "pucky_cover_wave";
    ...
    private static volatile CoverWaveController instance;
    ...
    private CoverWaveController(Context context) {
        this.context = context.getApplicationContext();
        this.prefs = this.context.getSharedPreferences(PREFS, Context.MODE_PRIVATE);
    }

    public static CoverWaveController shared(Context context) {
        CoverWaveController existing = instance;
        if (existing != null) {
            return existing;
        }
        synchronized (CoverWaveController.class) {
            if (instance == null) {
                instance = new CoverWaveController(context);
            }
            return instance;
        }
    }
    ...
    public JSONObject status() {
        ...
        synchronized (lock) {
            ...
            Json.put(out, "schema", "pucky.cover_wave_status.v1");
    ...
    public void start() {
        synchronized (lock) {
            ...
            sensorThread = new HandlerThread("PuckyCoverWave");
            sensorThread.start();
            ...
            if (registeredSensors.isEmpty()) {
                lastError = "No cover wave sensors registered";
                stopLocked();
    ...
    private void evaluateCandidate(long candidateId, long durationMs, String sourceSensor, float[] values) {
        ...
        String rejectReason = rejectionReason(preflight, accelGate);
    ...
    private String rejectionReason(PreflightSnapshot preflight, AccelGate accelGate) {
    ...
    private void startPreflight(long candidateId, String reason) {
        ...
        }, "pucky-cover-wave-preflight").start();
    }
    ...
    private void runDisplayAction(String action, String reason) {
        ...
        new Thread(() -> {
            ...
            if (wakeCallbacks != null) {
                wakeCallbacks.onCoverWaveWake(reason);
            }
        }, "pucky-cover-wave-display-" + action).start();
    }
