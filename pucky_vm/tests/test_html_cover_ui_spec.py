...
def test_inbox_manage_select_keeps_visible_dark_mode_chrome() -> None:
    ...
    inbox_card_menu = css_block(styles, ".inbox-card-menu-button")
    assert "border: 1px solid" in inbox_card_menu
    assert "color: var(--home-shell-text-muted);" in inbox_card_menu
    assert "background: color-mix(in srgb, var(--surface-card-elevated)" in inbox_card_menu
...
def test_inbox_management_uses_visible_archive_controls_without_delete_ui() -> None:
    ...
    can_manage = function_block(app, "canManageInboxCard")
    assert "function shouldShowInboxCardEscapeMenu(" in app
    assert "function cardOverflowMenu(" in app
    assert "function inboxCardMenuButton(" in app
    assert "function openInboxCardMenu(" in app
    assert "function isInboxCardMenuOpen(" in app
    assert 'Boolean(card.synthetic_pending) && !isMeetingProcessingCard(card) && !isFailedPendingOutboundCard(card)' in can_manage
    assert 'if (state.showArchivedFeed || Boolean(card?.archived)) {' in function_block(app, "shouldShowInboxCardEscapeMenu")
    assert 'isMeetingProcessingCard(card) || isFailedPendingOutboundCard(card)' in function_block(app, "shouldShowInboxCardEscapeMenu")
    assert 'addItem("Archive", "archive_folder", "archive"' in function_block(app, "cardOverflowMenu")
    ...
    assert 'wrapper.classList.add("has-inbox-menu");' in card_view
    ...
    assert ".card-wrap.has-inbox-menu" in styles
